...
In this document we briefly collect what we have learned while developing and using Murena,
...
useful for small projects to get going fast, it is not wise for more serious projects. In Murena,
...
while at the same time they often need to be customizable and exposed to users. In Murena we address these conflicting 
...
We test most aspects of Murena by having a small "project" for each supported language in `tests/resources`.
...
The [multilspy](https://github.com/microsoft/multilspy/) project helped us a lot in getting started and stands at the core of Murena.
...
We mitigate this through the GUI window and the dashboard, so the user sees whether Murena is running
