# Scala Setup Guide for Murena

This guide explains how to prepare a Scala project so that Murena can provide reliable code intelligence via Metals (Scala LSP) and how to run Scala tests manually.

Murena automatically bootstraps the Metals language server using Coursier when needed. Your project, however, must be importable by a build server (BSP) — typically via Bloop or sbt’s built‑in BSP — so that Metals can compile and index your code.
...
- Coursier command (`cs`) or the legacy `coursier` launcher
  - Murena uses `cs` if available; if only `coursier` exists, it will attempt to install `cs`. If neither is present, install Coursier first.
...
4. Once the import completes, start Murena in your project root and use it.
...
4. Start Murena in your project root. Murena will bootstrap Metals (if not already present) and connect to the build server using the configuration exported above.

---
## Using Murena with Scala

- Murena automatically detects Scala files (`*.scala`, `*.sbt`) and will start a Metals process per project when needed.
- On first run, you may see messages like “Bootstrapping metals…” in the Murena logs — this is expected.
