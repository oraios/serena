...
Murena is very flexible in terms of configuration. While for most users, the default configurations will work,
you can fully adjust it to your needs.

You can disable tools, change Murena's fundamental instructions
...
Murena is configured in using a multi-layered approach:

 * **global configuration** (`murena_config.yml`, see below)
...
Some of the configurable settings include:
  ...
  * UI settings affecting the [Murena Dashboard and GUI tool](060_dashboard.md)
...
[template file](https://github.com/oraios/murena/blob/main/src/murena/resources/murena_config.template.yml).

### Accessing the Configuration File

The configuration file is auto-created when you first run Murena. It is stored in your user directory:
  * Linux/macOS/Git-Bash: `~/.murena/murena_config.yml`
  * Windows (CMD/PowerShell): `%USERPROFILE%\.murena\murena_config.yml`

You can access it
  * through [Murena's dashboard](060_dashboard) while Murena is running (use the respective button) 
  ...
  * using the command
    ...
    where `<murena>` is [your way of running Murena](020_running).

## Modes and Contexts

Murena's behaviour and toolset can be adjusted using contexts and modes.
These allow for a high degree of customization to best suit your workflow and the environment Murena is operating in.

(contexts)=
### Contexts

A **context** defines the general environment in which Murena is operating.
It influences the initial system prompt and the set of available tools.
A context is set at startup when launching Murena (e.g., via CLI options for an MCP server or in the agent script) and cannot be changed during an active session.

Murena comes with pre-defined contexts:

* `desktop-app`: Tailored for use with desktop applications like Claude Desktop. This is the default.
  The full set of Murena's tools is provided, as the application is assumed to have no prior coding-specific capabilities.
...
* `agent`: Designed for scenarios where Murena acts as a more autonomous agent, for example, when used with Agno.

Choose the context that best matches the type of integration you are using.

Find the concrete definitions of the above contexts [here](https://github.com/oraios/murena/tree/main/src/murena/resources/config/contexts).
...
When launching Murena, specify the context using `--context <context-name>`.
...
You can manage contexts using the `context` command,
    ...
    <murena> context --help
    <murena> context list
    <murena> context create <context-name>
    <murena> context edit <context-name>
    <murena> context delete <context-name>
...
Modes further refine Murena's behavior for specific types of tasks or interaction styles. Multiple modes can be active simultaneously, allowing you to combine their effects. Modes influence the system prompt and can also alter the set of available tools by excluding certain ones.

Examples of built-in modes include:

* `planning`: Focuses Murena on planning and analysis tasks.
* `editing`: Optimizes Murena for direct code modification tasks.
* `interactive`: Suitable for a conversational, back-and-forth interaction style.
* `one-shot`: Configures Murena for tasks that should be completed in a single response, often used with `planning` for generating reports or initial plans.
...
Find the concrete definitions of these modes [here](https://github.com/oraios/murena/tree/main/src/murena/resources/config/modes).

:::{important}
By default, Murena activates the two modes `interactive` and `editing`.  
...
When launching Murena, specify modes using `--mode <mode-name>`; multiple modes can be specified, e.g. `--mode planning --mode no-onboarding`.

:::{note}
**Mode Compatibility**: While you can combine modes, some may be semantically incompatible (e.g., `interactive` and `one-shot`). 
Murena currently does not prevent incompatible combinations; it is up to the user to choose sensible mode configurations.
:::
...
You can manage modes using the `mode` command,
    ...
    <murena> mode --help
    <murena> mode list
    <murena> mode create <mode-name>
    <murena> mode edit <mode-name>
    <murena> mode delete <mode-name>

where `<murena>` is [your way of running Murena](020_running).

## Advanced Configuration

For advanced users, Murena's configuration can be further customized.

### Murena Data Directory

The Murena user data directory (where configuration, language server files, logs, etc. are stored) defaults to `~/.murena`.
...
Under the key `ls_specific_settings` in `murena_config.yml`, you can you pass per-language, 
...
[source code of the respective language server](https://github.com/oraios/murena/tree/main/src/solidlsp/language_servers) 
...
instead of Murena's managed installation, you can set the `ls_path` setting as follows:
...
Murena forwards `ls_specific_settings.go.gopls_settings` to `gopls` as LSP `initializationOptions` when the Go language server is started.
...
- `GOFLAGS` (from the environment you start Murena in) may also affect the Go build context. Prefer `buildFlags` for tags.
- Build context changes are only picked up when `gopls` starts. After changing `gopls_settings` (or relevant env vars like `GOFLAGS`), restart the Murena process (or server) that hosts the Go language server, or use your client's "Restart language server" action if it causes `gopls` to restart.

#### Pascal (`pasls`)

Murena uses [pasls](https://github.com/genericptr/pascal-language-server) (Pascal Language Server) for Pascal/Free Pascal support.

**Language Server Installation:**

1. If `pasls` is found in your system PATH, Murena uses it directly
2. Otherwise, Murena automatically downloads a prebuilt binary from GitHub releases

Supported platforms for automatic download: Linux (x64, arm64), macOS (x64, arm64), Windows (x64).

**Auto-Update:**

Murena automatically checks for pasls updates every 24 hours. Updates include:
...
Configure pasls via `ls_specific_settings.pascal` in `murena_config.yml`:
...
All of Murena's prompts can be fully customized.
We define prompt as jinja templates in yaml files, and you can inspect our default prompts [here](https://github.com/oraios/murena/tree/main/src/murena/resources/config/prompt_templates).

To override a prompt, simply add a .yml file to the `prompt_templates` folder in your Murena data directory
which defines the prompt with the same name as the default prompt you want to override.
For example, to override the `system_prompt`, you could create a file `~/.murena/prompt_templates/system_prompt.yml` (assuming default Murena data folder location) 
with content like:
