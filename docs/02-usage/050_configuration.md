...
| `null_analysis_mode` | `automatic` | How JDTLS handles null analysis (`java.compile.nullAnalysis.mode`), which drives the nullability diagnostics reported by `get_diagnostics_for_file`. `automatic` enables the required compiler options by itself once nullability annotations (`javax.annotation`, `org.eclipse.jdt.annotation`, `org.springframework.lang`) are on the classpath. `interactive` is JDTLS' own default: it asks the client before enabling those options, a prompt Serena never answers, so null analysis effectively stays off. `disabled` turns it off entirely — use this if nullability warnings are noise in your project, or to avoid the compiler-option changes `automatic` applies to the project. An unrecognized value logs a warning and falls back to `automatic`. The recognized annotations themselves are not configurable. |
