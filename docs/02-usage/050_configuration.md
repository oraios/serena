...
| `custom_jre_path` | `null` | Path to a Java executable used to launch JDTLS instead of Serena's bundled JRE. Use this when the bundled JRE is too old for a JDTLS bundle or project; a path under `<jdk>/bin/java` also sets the matching `JAVA_HOME`. |
