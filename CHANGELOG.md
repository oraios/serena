...
* Language Servers:
  ...
  - Add **CSS** (experimental) using `vscode-css-language-server` from the `vscode-langservers-extracted` npm package. Reports rules/selectors as document symbols; cross-file `@import` navigation is limited. For SCSS/Sass, use `scss` instead.
  - Add **SCSS / Sass** (experimental) using [some-sass-language-server](https://github.com/wkillerud/some-sass). Provides full `@use`/`@forward` workspace-wide go-to-definition and find-references for variables, mixins, and functions across `.scss` / `.sass` files.
