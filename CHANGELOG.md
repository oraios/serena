...
* Language Servers:
  ...
  - Java (`eclipse.jdt.ls`): Surface Lombok-generated methods (`getX/setX`, `builder()`, `equals/hashCode/toString`, `withX`, fluent accessors, etc.) in `find_symbol`, `get_symbols_overview`, `replace_symbol_body`, `insert_before_symbol` and `insert_after_symbol`. Previously JDTLS filtered `SourceMethod`s where `isGenerated()` is `true` out of `documentSymbol`/`workspace/symbol` results, so Lombok-synthesised methods were unreachable from any symbol-aware tool — only `search_for_pattern`/`replace_content` could touch them. Bumped bundled `vscode-java` from `1.42.0-561` to `1.54.0-923` (brings JDTLS commit `b2d8952` which introduces `java.symbols.includeGeneratedCode`, plus JRE 21.0.10, Lombok 1.18.39 and Equinox launcher 1.7.100); per the per-version install-dir convention from #1438, the new build lands in `vscode-java-1.54.0-923/`, so legacy `vscode-java/` caches with the original 1.42.0-561 paths (JRE 21.0.7, Lombok 1.18.36, launcher 1.7.0) are kept usable. New `lombok_show_generated` setting in `ls_specific_settings.java` (default `true`) toggles the JDTLS filter; set to `false` to restore the previous behaviour for `@Data`-heavy classes where the extra getters/setters are noise. Issue #1432.
