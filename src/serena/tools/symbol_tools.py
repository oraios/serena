...
class SafeDeleteSymbol(Tool, ToolMarkerSymbolicEdit):
    def apply(
        ...
        symbol_location = symbol.get_body_start_position_or_raise()
        lang_server = ls_symbol_retriever.get_language_server(symbol_rel_path)
        references_locations = lang_server.request_references(symbol_rel_path, symbol_location.line, symbol_location.col)
