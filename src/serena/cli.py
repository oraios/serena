...
def find_project_root(root: str | Path | None = None) -> str | None:
    ...
    Checks for .serena first (explicit Serena project), then .git (git root).
    ...
    :return: absolute path to project root, or None if not found
    """
    current = Path.cwd().resolve()
    boundary = Path(root).resolve() if root else None
...
class TopLevelCommands(AutoRegisteringGroup):
    ...
    @click.option(
        ...
        is_flag=True,
        default=False,
        help="Auto-detect project from current working directory (searches for .serena or .git).",
    )
    def start_mcp_server(
        ...
        # Handle --auto-project flag
        if auto_project:
            if project is not None or project_file_arg is not None:
                raise click.UsageError("--auto-project cannot be used with --project or positional project argument")
