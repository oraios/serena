...
class DelphiLanguageServer(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(cls, config: LanguageServerConfig, solidlsp_settings: SolidLSPSettings) -> str:
        ...
        Raises:
            FileNotFoundError: If DelphiLSP cannot be found

        """
        delphilsp_path = cls._find_delphilsp()
        if delphilsp_path:
            return quote_windows_path(delphilsp_path)

        raise FileNotFoundError(
            "DelphiLSP.exe not found. Requires RAD Studio 11.0+. "
            "See: https://docwiki.embarcadero.com/RADStudio/Alexandria/en/Using_DelphiLSP_Code_Insight_with_Other_Editors"
        )

    @classmethod
    def _find_delphilsp(cls) -> Optional[str]:
        """
        Find existing DelphiLSP installation.

        Returns:
            Optional[str]: Path to DelphiLSP.exe if found, None otherwise
