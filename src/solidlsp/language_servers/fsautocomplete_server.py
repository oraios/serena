...
class FsAutoCompleteServer(SolidLanguageServer):
    ...
    def _start_server(self) -> None:
        ...
        try:
            ...
            if self.fsac_version:
                ...
                self.logger.log("Could not determine FsAutoComplete server version.", logging.WARNING)
        ...
        if missing:
            self.logger.log(f"Warning: Language server is missing some capabilities: {', '.join(missing)}", logging.WARNING)
        ...
        if solution_or_project:
            ...
            self.server.notify.send_notification("solution/open", {"solution": PathUtils.path_to_uri(solution_or_project)})
        if project_files:
        ...
        if not self.workspace_loaded.wait(timeout=10):
            self.logger.log("Timeout waiting for FsAutoComplete workspace load signal. Proceeding anyway.", logging.WARNING)
    ...
    def request_hover(self, relative_file_path: str, line: int, column: int) -> ls_types.Hover | None:
        ...
        return self._retry_no_check_results(
            lambda: SolidLanguageServer.request_hover(self, relative_file_path, line, column)
        )
    ...
    def request_definition(self, relative_file_path: str, line: int, column: int) -> list[Any] | None:
        ...
        """
        return self._retry_no_check_results(
            lambda: SolidLanguageServer.request_definition(self, relative_file_path, line, column)
        )
    ...
    def _fix_selection_range(
        ...
        if start_line + 1 < len(file_lines) and file_lines[start_line].strip() == "" and file_lines[start_line + 1].lstrip().startswith("///"):
            shift = 1
