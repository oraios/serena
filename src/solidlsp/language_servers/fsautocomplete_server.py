...
def breadth_first_file_scan(root_dir: str, is_ignored_path: Callable[[str], bool] | None = None) -> Generator[str, None, None]:
    """
    ...
    :param is_ignored_path: A predicate that returns True if a relative path should be ignored.
    """
    # Use a list of (abs_path, rel_path) tuples
    queue = [(root_dir, ".")]

    while queue:
        current_abs, current_rel = queue.pop(0)
        ...
        for item in entries:
            ...
            if is_ignored_path and is_ignored_path(item_rel):
                continue

            if os.path.isdir(item_abs):
                queue.append((item_abs, item_rel))
            elif os.path.isfile(item_abs):
                yield item_abs


def find_solution_or_project_file(root_dir: str, is_ignored_path: Callable[[str], bool] | None = None) -> str | None:
    """
    ...
    for filename in breadth_first_file_scan(root_dir, is_ignored_path=is_ignored_path):
...
def find_all_project_files(root_dir: str, is_ignored_path: Callable[[str], bool] | None = None) -> list[str]:
    """
...
class FsAutoCompleteServer(SolidLanguageServer):
    ...
    def __init__(self, config: LanguageServerConfig, repository_root_path: str, solidlsp_settings: SolidLSPSettings):
        ...
        log.info(f"Using FsAutoComplete at: {fsautocomplete_path}")
    ...
    def is_ignored_dirname(self, dirname: str) -> bool:
        return super().is_ignored_dirname(dirname) or dirname in ["bin", "obj", "packages", ".fake", ".ionide", "paket-files"]
    ...
    def _start_server(self) -> None:
        ...
        def handle_notify_workspace(params: Any) -> None:
            ...
            try:
                ...
                if kind == "workspaceLoad":
                    ...
                    if status == "finished":
                        log.info("FsAutoComplete workspace loading finished")
                        self.workspace_loaded.set()
            except Exception as e:
                log.warning(f"Error parsing fsharp/notifyWorkspace: {e}")
        ...
        log.info("Starting FsAutoComplete language server process")

        try:
            self.server.start()
        except Exception as e:
            log.error(f"Failed to start language server process: {e}")
        ...
        try:
            ...
            # Cast to dict to avoid "object has no attribute get" mypy error
            init_resp_dict = cast(dict[str, Any], init_response)
            self.fsac_version = init_resp_dict.get("serverInfo", {}).get("version")
            if self.fsac_version:
                log.info(f"FsAutoComplete server version: {self.fsac_version}")
            else:
                log.warning("Could not determine FsAutoComplete server version.")
        ...
        capabilities = init_resp_dict.get("capabilities", {})
        ...
        if missing:
            log.warning(f"Warning: Language server is missing some capabilities: {', '.join(missing)}")

        # Complete initialization
        self.server.notify.initialized({})
        self.completions_available.set()

        # We use a custom ignore predicate to respect .gitignore but allow .sln/.fsproj files
        # (which are technically not "supported source files" for the LS, but needed for initialization)
        def is_project_file_ignored(path: str) -> bool:
            return self.is_ignored_path(path, ignore_unsupported_files=False)

        # Try to find a solution file first
        solution_file = find_solution_or_project_file(self.repository_root_path, is_ignored_path=is_project_file_ignored)

        # If find_solution_or_project_file returns a .fsproj, it means no .sln was found.
        # But we want to be explicit here.

        # Let's check specifically for a .sln first to be clean/standard
        sln_file = None
        # We can reuse the find_solution_or_project_file logic or just use it as is.
        # The helper returns the SLN if found, or the first fsproj if not.
        if solution_file and solution_file.endswith(".sln"):
            sln_file = solution_file

        if sln_file:
            log.info(f"Opening solution file: {sln_file}")
            self.server.notify.send_notification("solution/open", {"solution": PathUtils.path_to_uri(sln_file)})
        else:
            # No solution found, fallback to opening all project files
            project_files = find_all_project_files(self.repository_root_path, is_ignored_path=is_project_file_ignored)
            if project_files:
                log.info(f"No .sln found. Opening {len(project_files)} project files")
                ...
                log.warning("No solution or project files found.")

        # Wait for workspace load signal instead of polling
        if not self.workspace_loaded.wait(timeout=10):
            log.warning("Timeout waiting for FsAutoComplete workspace load signal. Proceeding anyway.")

        log.info("FsAutoComplete language server initialized and ready")
    ...
    def request_definition(self, relative_file_path: str, line: int, column: int) -> list[ls_types.Location]:
        ...
        return cast(
            ...
            self._retry_no_check_results(lambda: SolidLanguageServer.request_definition(self, relative_file_path, line, column)),
        )
    ...
    def _fix_selection_range(
        symbol: ls_types.UnifiedSymbolInformation, file_lines: list[str], fsac_version: str | None = None
        ...
        if shift != 0:
            ...
            apply(cast(dict[str, Any] | None, selection))
            apply(cast(dict[str, Any] | None, rng))
            apply(cast(dict[str, Any] | None, loc_rng))
        ...
        # Fix character column for identifier (selectionRange)
        # If selectionRange is on a line with the identifier, set the character column correctly
        sel_start_line = selection["start"]["line"]
        if 0 <= sel_start_line < len(file_lines):
            line_content = file_lines[sel_start_line]
            identifier = symbol.get("name", "")
            if identifier:
                idx = line_content.find(identifier)
                if idx != -1:
                    # If range start was same as selection start (e.g. 0), update it too as per test expectation
                    if rng and rng["start"]["character"] == selection["start"]["character"]:
                        rng["start"]["character"] = idx

                    selection["start"]["character"] = idx
                    selection["end"]["character"] = idx + len(identifier)

        return symbol

    @staticmethod
    def _convert_ranges_to_one_based(symbols: list[ls_types.UnifiedSymbolInformation]) -> list[ls_types.UnifiedSymbolInformation]:
        """
        ...
        def recurse(sym: ls_types.UnifiedSymbolInformation) -> ls_types.UnifiedSymbolInformation:
            bump_range(cast(dict[str, Any] | None, sym.get("range")))
            bump_range(cast(dict[str, Any] | None, sym.get("selectionRange")))
            if "location" in sym:
                bump_range(cast(dict[str, Any] | None, sym["location"].get("range")))
    ...
    @staticmethod
    def _align_ranges_to_selection(symbols: list[ls_types.UnifiedSymbolInformation]) -> list[ls_types.UnifiedSymbolInformation]:
        """
    ...
    def _fix_selection_ranges(
        cls, symbols: list[ls_types.UnifiedSymbolInformation], file_lines: list[str], fsac_version: str | None = None
    ) -> list[ls_types.UnifiedSymbolInformation]:
        fixed: list[ls_types.UnifiedSymbolInformation] = []
        for sym in symbols:
            fixed_sym = cls._fix_selection_range(sym, file_lines, fsac_version)
            if fixed_sym.get("children"):
                fixed_sym["children"] = cls._fix_selection_ranges(fixed_sym["children"], file_lines, fsac_version)
    ...
    def request_document_symbols(self, relative_file_path: str, file_buffer: LSPFileBuffer | None = None) -> DocumentSymbols:
        ...
        fixed_root_symbols = self._fix_selection_ranges(document_symbols.root_symbols, file_lines, self.fsac_version)
