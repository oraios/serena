...
class PascalLanguageServer(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(cls, config: LanguageServerConfig, solidlsp_settings: SolidLSPSettings) -> str:
        ...
        """
        pasls_path = cls._find_pasls(solidlsp_settings)
        if pasls_path:
            return quote_windows_path(pasls_path)

        # pasls not found, ensure it's installed
        pasls_path = cls._ensure_pasls_installed(solidlsp_settings)
        return quote_windows_path(pasls_path)

    @classmethod
    def _find_pasls(cls, solidlsp_settings: SolidLSPSettings) -> Optional[str]:
        """
        Find existing pasls installation.

        Returns:
            Optional[str]: Path to pasls executable if found, None otherwise
        ...
        if os.path.exists(pasls_executable):
            ...
            return pasls_executable

        return None

    @classmethod
    def _ensure_pasls_installed(cls, solidlsp_settings: SolidLSPSettings) -> str:
        """
        Ensure pasls is installed, building from source if necessary.

        Returns:
            str: Path to the installed pasls executable

        Raises:
            FileNotFoundError: If pasls cannot be built or found
        ...
        # Attempt to build pasls
        log.info("pasls not found. Building from source...")
        cls._build_pasls(pasls_dir, solidlsp_settings)

        if not os.path.exists(pasls_executable):
            raise FileNotFoundError(
                "Failed to build pasls. "
                "Please ensure Free Pascal Compiler (fpc) and Lazarus are installed, "
                "or manually compile pasls and add it to PATH. "
                "See: https://github.com/genericptr/pascal-language-server"
            )
    ...
    def _build_pasls(cls, target_dir: str, solidlsp_settings: SolidLSPSettings) -> None:
        ...
        if not lazbuild:
            raise FileNotFoundError("lazbuild not found. Please install Lazarus or add lazbuild to PATH. See: https://www.lazarus-ide.org/")
        ...
        if not os.path.exists(pasls_source_dir):
            ...
            subprocess.run([git, "clone", pasls_repo_url, pasls_source_dir], check=True, capture_output=True, text=True)
        else:
        ...
        log.info("Building pasls using lazbuild...")
        try:
            result = subprocess.run([lazbuild, pasls_project_file], check=True, capture_output=True, text=True, cwd=pasls_source_dir)
    ...
    def _get_initialize_params(repository_absolute_path: str) -> InitializeParams:
        ...
        # Only pass target OS/CPU if explicitly set
        fpctarget = os.environ.get("FPCTARGET", "")
        fpctargetcpu = os.environ.get("FPCTARGETCPU", "")

        # Build initializationOptions
        initialization_options: dict = {}
        if fpctarget:
            initialization_options["FPCTARGET"] = fpctarget
        if fpctargetcpu:
            initialization_options["FPCTARGETCPU"] = fpctargetcpu
