...
from murena.text_utils import render_html
from murena.tools.jetbrains_types import PluginStatusDTO
from murena.util.version import Version

T = TypeVar("T")
log = logging.getLogger(__name__)


class MurenaClientError(Exception):
    """Base exception for Serena client errors."""


class ConnectionError(MurenaClientError):
    """Raised when connection to the service fails."""


class APIError(MurenaClientError):
...
class PluginServerError(MurenaClientError):
...
class JetBrainsPluginClient(ToStringMixin):
    ...
    def _make_request(self, method: str, endpoint: str, data: Optional[dict] = None) -> dict[str, Any]:
        ...
        try:
            ...
            raise MurenaClientError(f"Request failed: {e}")
