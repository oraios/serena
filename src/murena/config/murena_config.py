...
from murena.constants import (
    ...
    MURENA_CONFIG_TEMPLATE_FILE,
    MURENA_FILE_ENCODING,
    MURENA_MANAGED_DIR_NAME,
    PROJECT_TEMPLATE_FILE,
    REPO_ROOT,
)
from murena.util.general import get_dataclass_default, load_yaml, save_yaml
from murena.util.inspection import determine_programming_language_composition
...
class MurenaPaths:
    ...
    def __init__(self) -> None:
        home_dir = os.getenv("MURENA_HOME")
        if home_dir is None or home_dir.strip() == "":
            home_dir = str(Path.home() / MURENA_MANAGED_DIR_NAME)
        else:
        ...
        This is ~/.serena by default, but it can be overridden via the MURENA_HOME environment variable.
        """
        ...
        If a name of a context matches a name of a context in MURENAS_OWN_CONTEXT_YAMLS_DIR, 
        ...
        If a name of a mode matches a name of a mode in MURENAS_OWN_MODES_YAML_DIR,
...
class MurenaConfigError(Exception):
    pass


def get_serena_managed_in_project_dir(project_root: str | Path) -> str:
    return os.path.join(project_root, MURENA_MANAGED_DIR_NAME)
...
class ProjectConfig(ToolInclusionDefinition, ToStringMixin):
    ...
    MURENA_DEFAULT_PROJECT_FILE = "project.yml"
            ...
            if languages is None:
                ...
                if len(language_composition) == 0:
                    ...
                    raise ValueError(
                        ...
                        f"     when creating the project via the Murena CLI command OR\n"
    ...
    def rel_path_to_project_yml(cls) -> str:
        return os.path.join(MURENA_MANAGED_DIR_NAME, cls.MURENA_DEFAULT_PROJECT_FILE)
...
class RegisteredProject(ToStringMixin):
    def __init__(self, project_root: str, project_config: "ProjectConfig", project_instance: Optional["Project"] = None) -> None:
        """
        Represents a registered project in the Murena configuration.
...
class CacheConfig:
    """
    Configuration for cache behavior (async persistence, LRU limits, etc.).
    """

    # Async cache persistence settings
    async_persistence_enabled: bool = True
    """Whether to use asynchronous cache persistence (non-blocking writes)"""

    async_persistence_debounce_interval: float = 5.0
    """Minimum time in seconds between cache writes for the same key (debouncing)"""

    # LRU cache settings
    lru_enabled: bool = True
    """Whether to use LRU caches with memory/entry limits (vs unbounded caches)"""

    lru_max_raw_symbols_entries: int = 1000
    """Maximum number of entries in raw document symbols LRU cache"""

    lru_max_raw_symbols_memory_mb: int = 200
    """Maximum memory usage in MB for raw document symbols LRU cache"""

    lru_max_document_symbols_entries: int = 1000
    """Maximum number of entries in document symbols LRU cache"""

    lru_max_document_symbols_memory_mb: int = 200
...
class MurenaConfig(ToolInclusionDefinition, ToStringMixin):
    ...
    """
    default_max_tool_answer_chars: int = 60_000
    ...
    def _generate_config_file(cls, config_file_path: str) -> None:
        """
        Generates a Murena configuration file at the specified path from the template file.
    ...
    def _determine_config_file_path(cls) -> str:
        """
        :return: the location where the Murena configuration file is stored/should be stored
        """
        config_path = os.path.join(MurenaPaths().serena_user_home_dir, cls.CONFIG_FILE)
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists(old_config_path):
                log.info(f"Moving Murena configuration file from {old_config_path} to {config_path}")
    ...
    @classmethod
    def from_config_file(cls, generate_if_missing: bool = True) -> "MurenaConfig":
        """
        Static constructor to create MurenaConfig from the configuration file
        """
        ...
        if not os.path.exists(config_file_path):
            if not generate_if_missing:
                raise FileNotFoundError(f"Murena configuration file not found: {config_file_path}")
            log.info(f"Murena configuration file not found at {config_file_path}, autogenerating...")
            cls._generate_config_file(config_file_path)

        # load the configuration
        log.info(f"Loading Murena configuration from {config_file_path}")
        try:
        ...
        def get_value_or_default(field_name: str) -> Any:
            ...
            if field_name not in loaded_commented_yaml:
                num_migrations += 1
            return loaded_commented_yaml.get(field_name, get_dataclass_default(MurenaConfig, field_name))
        ...
        if "projects" not in loaded_commented_yaml:
            raise MurenaConfigError("`projects` key not found in Murena configuration. Please update your `murena_config.yml` file.")
        instance.projects = []
        ...
        language_backend = get_dataclass_default(MurenaConfig, "language_backend")
    ...
    def _migrate_out_of_project_config_file(cls, path: Path) -> Path | None:
        ...
        try:
            ...
            if "project_name" not in project_config_data:
                ...
                with open(path, "a", encoding=MURENA_FILE_ENCODING) as f:
    ...
    def add_project_from_path(self, project_root: Path | str) -> "Project":
        """
        Add a new project to the Murena configuration from a given path, auto-generating the project
    ...
    def remove_project(self, project_name: str) -> None:
        ...
        for i, project in enumerate(list(self.projects)):
            ...
            raise ValueError(f"Project '{project_name}' not found in Murena configuration; valid project names: {self.project_names}")
        self.save()
    ...
    def propagate_settings(self) -> None:
        ...
        """
        from murena.tools import JetBrainsPluginClient
