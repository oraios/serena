...
from murena.config.murena_config import MurenaPaths, ToolInclusionDefinition
...
class MurenaAgentMode(ToolInclusionDefinition, ToStringMixin):
    ...
    It is formatted by the agent (see MurenaAgent._format_prompt()).
    """
    description: str = ""
    ...
    def from_yaml(cls, yaml_path: str | Path) -> Self:
        ...
        with Path(yaml_as_path).open(encoding=MURENA_FILE_ENCODING) as f:
    ...
    def get_path(cls, name: str, instance: Self | None = None) -> str:
        ...
        fname = f"{name}.yml"
        custom_mode_path = os.path.join(MurenaPaths().user_modes_dir, fname)
        if os.path.exists(custom_mode_path):
            return custom_mode_path

        own_yaml_path = os.path.join(MURENAS_OWN_MODE_YAMLS_DIR, fname)
        if not os.path.exists(own_yaml_path):
            raise FileNotFoundError(
                f"Mode {name} not found in {MurenaPaths().user_modes_dir} or in {MURENAS_OWN_MODE_YAMLS_DIR}."
    ...
    def list_custom_mode_names(cls) -> list[str]:
        ...
        return [f.stem for f in Path(MurenaPaths().user_modes_dir).glob("*.yml")]
...
@dataclass(kw_only=True)
class MurenaAgentContext(ToolInclusionDefinition, ToStringMixin):
    ...
    def from_yaml(cls, yaml_path: str | Path) -> Self:
        ...
        yaml_as_path = Path(yaml_path).resolve()
        with yaml_as_path.open(encoding=MURENA_FILE_ENCODING) as f:
            data = yaml.safe_load(f)
        name = data.pop("name", yaml_as_path.stem)
    ...
    def get_path(cls, name: str, instance: Self | None = None) -> str:
        ...
        fname = f"{name}.yml"
        custom_context_path = os.path.join(MurenaPaths().user_contexts_dir, fname)
        if os.path.exists(custom_context_path):
            return custom_context_path

        own_yaml_path = os.path.join(MURENAS_OWN_CONTEXT_YAMLS_DIR, fname)
        if not os.path.exists(own_yaml_path):
            raise FileNotFoundError(
                f"Context {name} not found in {MurenaPaths().user_contexts_dir} or in {MURENAS_OWN_CONTEXT_YAMLS_DIR}."
    ...
    def list_custom_context_names(cls) -> list[str]:
        ...
        return [f.stem for f in Path(MurenaPaths().user_contexts_dir).glob("*.yml")]
