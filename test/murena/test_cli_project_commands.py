...
from murena.cli import ProjectCommands, TopLevelCommands, find_project_root
...
class TestProjectCreate:
    ...
    def test_create_basic_with_language(self, cli_runner, temp_project_dir):
        ...
        yml_path = os.path.join(temp_project_dir, ".murena", "project.yml")
    ...
    def test_create_auto_detect_language(self, cli_runner, temp_project_dir_with_python_file):
        ...
        # Verify project.yml was created
        yml_path = os.path.join(temp_project_dir_with_python_file, ".murena", "project.yml")
    ...
    def test_create_with_index_flag(self, cli_runner, temp_project_dir_with_python_file):
        ...
        cache_dir = os.path.join(temp_project_dir_with_python_file, ".murena", "cache")
    ...
    def test_create_without_index_flag(self, cli_runner, temp_project_dir):
        ...
        cache_dir = os.path.join(temp_project_dir, ".murena", "cache")
...
class TestProjectIndex:
    ...
    def test_index_is_equivalent_to_create_with_index(self, cli_runner, temp_project_dir_with_python_file):
        ...
        try:
            ...
            assert os.path.exists(os.path.join(dir1, ".murena", "project.yml"))
            assert os.path.exists(os.path.join(dir2, ".murena", "project.yml"))

            # Both should create cache (proof of indexing)
            assert os.path.exists(os.path.join(dir1, ".murena", "cache"))
            assert os.path.exists(os.path.join(dir2, ".murena", "cache"))
        finally:
...
class TestFindProjectRoot:
    ...
    def test_finds_serena_from_subdirectory(self, temp_project_dir):
        """Test that .murena/project.yml is found when searching from a subdirectory."""
        serena_dir = os.path.join(temp_project_dir, ".murena")
    ...
    def test_serena_preferred_over_git(self, temp_project_dir):
        """Test that .murena/project.yml takes priority over .git at the same level."""
        serena_dir = os.path.join(temp_project_dir, ".murena")
        os.makedirs(serena_dir)
        Path(os.path.join(serena_dir, "project.yml")).touch()
        ...
        try:
            ...
            result = find_project_root(root=temp_project_dir)
            assert result is not None
            assert os.path.isdir(os.path.join(result, ".murena"))
            assert os.path.samefile(result, temp_project_dir)
        finally:
            os.chdir(original_cwd)
