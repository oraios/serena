...
from murena.tools.tools_base import ToolRegistry
...
def test_all_tool_parameters_have_type(context):
    ...
    """
    cfg = MurenaConfig(gui_log_window=False, web_dashboard=False, log_level=logging.ERROR)
    registry = ToolRegistry()
    cfg.included_optional_tools = tuple(registry.get_tool_names_optional())
    factory = MurenaMCPFactory(context=context)
