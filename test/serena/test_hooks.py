...
class TestPreToolUseEnforceSymbolicToolsHook:
    ...
    def test_codex_grep_without_code_file_target_keeps_pattern_only(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        hook_output = output["hookSpecificOutput"]
        assert hook_output["permissionDecision"] == "deny"
        assert 'mcp__serena__search_for_pattern(substring_pattern="needle")' in hook_output["permissionDecisionReason"]

    def test_codex_grep_skips_flag_values_when_extracting_pattern(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        """``--type py`` must not be mistaken for the search pattern."""
        payload = _base_input(tool_name="exec_command", tool_input={"cmd": "rg -n --type py needle src/foo.py"})
        _execute_enforce_hook(HookClient.CODEX, payload, tmp_path)
