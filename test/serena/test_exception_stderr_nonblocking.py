...
def test_fatal_print_survives_blocking_io_error_on_stderr() -> None:
    ...
    with patch("serena.util.exception.log", log), patch("serena.util.exception.is_headless_environment", return_value=True), patch.object(
        sys, "stderr", _ExplodingStderr()
    ):
        # Must not raise
