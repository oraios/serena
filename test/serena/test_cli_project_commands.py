...
class TestAutoProjectMutualExclusivity:
    """Tests for --auto-project mutual exclusivity."""

    def test_auto_project_with_project_flag_fails(self, cli_runner):
        """Test that --auto-project with --project raises error."""
        result = cli_runner.invoke(
            TopLevelCommands.start_mcp_server,
            ["--auto-project", "--project", "/some/path"],
        )
        assert result.exit_code != 0
