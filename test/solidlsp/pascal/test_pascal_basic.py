...
class TestPascalLanguageServerBasics:
    ...
    def test_pascal_request_document_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        function_symbols = [s for s in all_symbols if s.get("kind") == SYMBOL_KIND_FUNCTION]
        ...
        class_symbols = [s for s in all_symbols if s.get("kind") == SYMBOL_KIND_CLASS]
    ...
    def test_pascal_class_methods(self, language_server: SolidLanguageServer) -> None:
        ...
        genericptr pasls returns methods with containerName field indicating parent class.
        """
        all_symbols, _root_symbols = language_server.request_document_symbols("main.pas").get_all_symbols_and_roots()

        # Get all function symbols (pasls returns class methods as functions with containerName)
        function_symbols = [s for s in all_symbols if s.get("kind") == SYMBOL_KIND_FUNCTION]
        function_names = [s["name"] for s in function_symbols]
        ...
        for method in expected_tuser_methods:
            # genericptr pasls returns methods as functions with containerName
            found = method in function_names
    ...
    def test_pascal_helper_unit_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        function_symbols = [s for s in helper_all_symbols if s.get("kind") == SYMBOL_KIND_FUNCTION]
        function_names = [s["name"] for s in function_symbols]
        ...
        # Should also detect THelper class methods
        assert "FormatString" in function_names, "Should find FormatString method"
        assert "IsEven" in function_names, "Should find IsEven method"
    ...
    def test_pascal_cross_file_references(self, language_server: SolidLanguageServer) -> None:
        ...
        helper_function_names = [s["name"] for s in helper_symbols if s.get("kind") == SYMBOL_KIND_FUNCTION]
