...
class TestHaskellLanguageServer:
    ...
    def test_hover_information(self, language_server: SolidLanguageServer) -> None:
        ...
        assert hover_info is None or (isinstance(hover_info, dict) and 'contents' in hover_info), f"Expected hover info dict with 'contents' or None, got: {type(hover_info)}"
    ...
    def test_main_file_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        assert has_symbols, f"Expected to find some symbols in Main.hs. Flat: {len(flat_symbols)}, Hierarchical: {len(hierarchical_symbols)}"
